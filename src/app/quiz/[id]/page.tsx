...
import { CheckCircle, XCircle, ArrowRight, ArrowLeft, Trophy, Clock, Target, Menu, X, Home, Sparkles, RotateCcw, ChevronRight, Share2 } from 'lucide-react';
...
export default function QuizQuestionPage() {
  ...
  const questionId = parseInt(params.id as string) - 1; // Convert to 0-based index
  
  // Validate question ID
  const currentIndex = questionId >= 0 && questionId < sampleQuizQuestions.length ? questionId : 0;
  ...
  const shareQuestion = async () => {
    const shareUrl = `${window.location.origin}/quiz/${currentIndex + 1}`;
    const shareData = {
      title: `JS Quiz - Question ${currentIndex + 1}`,
      text: `Try this JavaScript quiz question: ${currentQuiz.title}`,
      url: shareUrl
    };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="p-2 text-slate-400 hover:text-amber-400 rounded-xl hover:bg-slate-700/50 transition-colors"
              >
        ...
        <div className="hidden lg:block glass border-b border-slate-700/50 p-6 sticky top-0 z-30">
          <div className="flex items-center justify-between max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-6">
              <button
                onClick={shareQuestion}
                className="flex items-center gap-2 px-4 py-2 glass-light text-slate-300 rounded-xl hover:bg-slate-700/50 transition-all duration-200 text-sm font-medium"
              >
